...
if(!empty($firstname) || !empty($lastname)||!empty($address)|| !empty($phone)|| !empty($email) || !empty($departuredate) || !empty($arrivaldate) || !empty($location) || !empty($msg))
{
    ...
    { 
  $SELECT = "SELECT email from travelbooking Where email = ? Limit 1 "; 
        
   $INSERT = "INSERT Into travelbooking (firstname, lastname, address,phone, email, departuredate, arrivaldate, location,msg) values(?,?,?,?,?,?,?,?,?)";
    //prepared statement
        ...
        {            
            ...
            header("Location:travelbookingmain.php?msg1=".$msg1);
        }
        else
        {
